---
title: 'How Do You Come Up With a Workable Idea?'
date: 2024-07-02
...
Take an LLM, first really figure out prompting, then build a pile of API Agents: write a dating copilot, test some bias, mess around with social media accounts, and so on.

You build intuition while playing. Then read more related papers, write up what you played with, and there is your top-conference paper.
...
After that, you can even consider playing with pretraining and model architecture. But if you are short on GPUs, forget it. This path burns through a lot of cards.
...
Once you have this kind of understanding, it is as if you have a useful hammer in your hand and can go looking everywhere for nails. For the people at OpenAI, once scaling law was established, the only remaining question was what to scale. Luckily, they eventually found the perfect carrier: large language models.
...
RL = genetic algorithms = black-box optimization = zeroth-order optimization = nothing magical, so we should look for gradient information everywhere and use it instead.
