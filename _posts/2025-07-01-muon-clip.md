---
title: 'From Muon to Gradient Clipping: Notes on QK Stability'
date: 2025-07-01
permalink: /posts/2025/07/muon-clip/
tags: [AI, LLM, optimization]
---

## Prelude: an elegant theory and a stubborn problem

In the toolbox of deep learning, the optimizer is the engine. While most of us have become used to tuning Adam and its variants, another optimizer built on a rather different philosophy has gradually entered researchers' field of view: the Muon optimizer. Keller Jordan, Jeremy Bernstein, and collaborators first proposed it in their work [1]. Its depth lies in the fact that it no longer stays within the familiar parameter space. Instead, it starts from the perspective of function space and builds its update rule from there.

This idea has recently been pushed forward and successfully practiced in work such as Kimi K2 [7], showing substantial promise. Yet the public material around K2 also points out a difficult issue: if the original Muon optimizer is applied directly to the Query (Q) and Key (K) weight matrices of a Transformer, training can become extremely unstable, and can even collapse.
...
$$
\min_{\Delta W} \operatorname{Tr}(G^T \Delta W)
\quad \text{subject to a size constraint on } \Delta W.
$$
...
$$
\Delta S =
X(\Delta W_Q W_K^T + W_Q \Delta W_K^T + \Delta W_Q \Delta W_K^T)X^T.
$$
...
$$
\Delta L \approx
\operatorname{Tr}(G_Q^T \Delta W_Q) +
\operatorname{Tr}(G_K^T \Delta W_K),
$$
...
$$
\min_{\Delta W_Q} \operatorname{Tr}(G_Q^T \Delta W_Q)
\quad \text{subject to} \quad
\|\Delta W_Q W_K^T\|_2 \le \epsilon/2.
$$

Subproblem 2, for $\Delta W_K$:

$$
\min_{\Delta W_K} \operatorname{Tr}(G_K^T \Delta W_K)
\quad \text{subject to} \quad
\|W_Q \Delta W_K^T\|_2 \le \epsilon/2.
$$
...
$$
\min_Z \operatorname{Tr}\!\left( \left[G_Q ((W_K^T)^+)^T\right]^T Z \right)
\quad \text{subject to} \quad
\|Z\|_2 \le \epsilon/2.
$$
...
$$
Z^* = -\frac{\epsilon}{2}
\operatorname{msign}\!\left(G_Q ((W_K^T)^+)^T\right).
$$
...
\Delta W_Q^* =
-\frac{\epsilon}{2}
\operatorname{msign}\!\left(G_Q ((W_K^T)^+)^T\right)(W_K^T)^+.
$$
...
But when we leave the hall of mathematics and return to engineering reality, an enormous obstacle stands in the way: the pseudoinverse.
...
This is the first major setback. We have derived a theoretically impeccable weapon, but it is almost unusable in practice because of its computational complexity. The result is frustrating, but it clearly marks the gap between theory and practice. It also sets up the next question: can we bypass or simplify this frightening pseudoinverse computation?
...
At first glance, this argument looks like a pile of unrelated matrices fighting one another. But then we notice that the gradient $G_Q$ is not rootless. Its own structure is determined by the chain rule of backpropagation, and it is deeply related to $W_K$.
...
After the difficult derivation in section 2 and the "false" simplification in section 3, the exploration seems to have entered a dead end. The theoretically perfect formula is not engineerable. This dilemma forces us to take a step back and revisit the essence of the problem. A simple but fundamental question appears: why must we view $W_Q$ and $W_K$ separately?
...
$$
\min_{\Delta B} \operatorname{Tr}(G_B^T \Delta B)
\quad \text{subject to} \quad
\|\Delta B\|_2 \le \epsilon,
$$
...
This discovery is an accidental gain. It raises the value of solving the QK update problem. It is no longer merely a stability trick for training large models. It may open a new function-space optimization path for parameter-efficient fine-tuning.
...
Once we abandon the complicated analytic solution, we can intervene in several key quantities in the optimization process from a more direct engineering perspective. These interventions share the same goal: make the QK weight update smoother without adding too much computational overhead.
...
2. Clip the gradient matrix elementwise to obtain a clipped gradient, for example
   $G_S^{\text{clip}} = \operatorname{clip}(G_S, -c, c)$,
   where $c$ is a tunable hyperparameter.
...
$$
W_Q \leftarrow s^\alpha W_Q,
\quad
W_K \leftarrow s^{1-\alpha} W_K,
$$
...
[2] Su, Jianlin. (2025). "QK-Clip: Letting Muon Go Further on the Road to Scaleup." Scientific Spaces. Available at: <https://spaces.ac.cn/archives/11126>

[3] Su, Jianlin. (2025). "Muon Sequel: Why Did We Choose to Try Muon?" Scientific Spaces. Available at: <https://spaces.ac.cn/archives/10739>
